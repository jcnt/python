...
Istanbul,500,10,5,1000000`
	...
	for i, v := range cols {
		if len(os.Args) >= 2 && v == os.Args[1] {
			from = i
		}
		if len(os.Args) == 3 && v == os.Args[2] {
			to = i + 1
		}
		if len(os.Args) > 3 {
			fmt.Println("Only from and to please")
			return
		}
		if from > to {
			fmt.Println("from should be smaller than to")
			return
		}
	}

	for i := range rows {
		for _, v := range strings.Split(rows[i], separator)[from:to] {
			fmt.Printf("%-15s", v)
		}
		fmt.Println()
	}
