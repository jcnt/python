...
for k in sdict:
    ...
    if not exists:
        ...
        exists = False
    print("conn pairs", len(conn_pairs))

conn_pairs.remove([0, 0])
conn_pairs.sort(key=len, reverse=True)
print(sdict[lastconn - 1])
a = sdict[lastconn - 1][0].split(",")[0]
b = sdict[lastconn - 1][0].split(",")[1]
print(int(source[int(a)][0]) * int(source[int(b)][0]))
